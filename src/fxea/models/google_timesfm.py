...
def get_data(context_len: int,
  ...
  df = pd.read_csv(
    	"USDJPY_M1_2025_03_05.csv",
        header=None,
        names=["date","open", "high", "low", "close", "value"]
  )
  time_series = df["open"].diff().fillna(0).values
...
def finetune():
  ...
  # save
  torch.save(model.state_dict(), 'timesfm_fx_model.pth')

def get_forecast(input_df = [], value = "value", model_name = ""):
	if model_name == "":
